...
echo "=== 2/6 Generate Ed25519 keypair ON THIS NODE (private key never leaves) ==="
...
echo "=== 3/6 Install the signed-heartbeat writer (counter + measured_at INSIDE the signed bytes) ==="
...
# The exact bytes that are signed AND served (verifier checks sig over this string):
payload="{\"node\":\"tesseract-a\",\"status\":\"up\",\"counter\":$ctr,\"measured_at\":\"$now\"}"
...
# Embed payload as a JSON string so the verifier signs/verifies identical bytes:
...
${DOMAIN}, www.${DOMAIN} {
    ...
    handle {
        respond "Tesseract-A online" 200
    }
}
CADDY
...
echo ""
echo "--- PUBLIC KEY (commit this to pubkeys/${NODE}.pub in nexinus-bootstrap) ---"
